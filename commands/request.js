...
import { MessageActionRow, MessageSelectMenu, MessageMenuOption, MessageMenu, MessageButton } from 'discord.js';
...
export async function execute(interaction) {
	...
	if (Object.keys(searchResults).length > 0) {
		...
		for (var i = 0; i < maxResults; i++) {
			var object = new Object(), id, mediaType, title, overview, poster;
...
export async function search(id, interaction) {
	...
	function showBuilder() {
		try {
			...
			if (object.available)
				embed.addField('__Available__', '✅', true);
			if (object.requested)
				embed.addField('__Requested__', '✅', true);
...
export async function sendRequest(interaction, id, mediaType) {
	const { member } = interaction;
